...
const inferRootOptions = require('./utils/inferRootOptions')
const normalizeFilePaths = require('./utils/normalizeFilePaths')
const runCodemod = require('./utils/runCodemod')
const { toShortPluginId, matchesPluginId } = require('@megalo/cli-share-utils')
const ConfigTransform = require('./ConfigTransform')

const logger = require('@megalo/cli-share-utils/lib/logger')
const logTypes = {
  log: logger.log,
  info: logger.info,
  done: logger.done,
...
module.exports = class Generator {
  constructor (context, {
    ...
    this.scopedPkgVersions = scopedPkgVersions
    ...
    const cliService = plugins.find(p => p.id === '@megalo/cli-service')
  ...
  extractConfigFiles (extractAll, checkExisting) {
    ...
    if (extractAll) {
      ...
      if (!process.env.MEGALO_CLI_TEST) {
        // by default, always extract megalo.config.js
        extract('megalo')
      }
  ...
  async resolveFiles () {
    ...
    debug('megalo:cli-files')(this.files)
  }
