...
const { getPluginLink, toShortPluginId, loadModule } = require('@megalo/cli-share-utils')
...
class GeneratorAPI {
  ...
  constructor (id, generator, options, rootOptions) {
    ...
    this.pluginsData = generator.plugins
      .filter(({ id }) => id !== `@megalo/cli-service`)
      .map(({ id }) => ({
  ...
  assertCliVersion (range) {
    ...
    throw new Error(
      `Require global @megalo/cli "${range}", but was invoked by "${this.cliVersion}".`
    )
  }
  ...
  get cliServiceVersion () {
    ...
    if (process.env.MEGALO_CLI_TEST && process.env.MEGALO_CLI_SKIP_WRITE) {
      return this.cliVersion
    }

    const servicePkg = loadModule(
      '@megalo/cli-service/package.json',
  ...
  assertCliServiceVersion (range) {
    ...
    throw new Error(
      `Require @megalo/cli-service "${range}", but was loaded with "${this.cliServiceVersion}".`
    )
  }
   ...
   *
   * @param {string} id - Plugin id, can omit the (@megalo/|megalo-|@scope/megalo)-cli-plugin- prefix
   * @return {boolean}
   */
  hasPlugin (id) {
  ...
  addConfigTransform (key, options) {
    ...
    if (
      ...
      if (hasReserved) {
        const { warn } = require('@megalo/cli-share-utils')
