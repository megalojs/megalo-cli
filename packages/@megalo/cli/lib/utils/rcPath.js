...
const xdgConfigPath = file => {
  ...
  if (xdgConfigHome) {
    const rcDir = path.join(xdgConfigHome, 'megalo')
...
// we introduced a change storing .megalorc in AppData, but the benefit isn't
...
const migrateWindowsConfigPath = file => {
  ...
  if (appData) {
    const rcDir = path.join(appData, 'megalo')
...
exports.getRcPath = file => {
  ...
  return (
    process.env.MEGALO_CLI_CONFIG_PATH ||
    xdgConfigPath(file) ||
